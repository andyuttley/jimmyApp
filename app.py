...
##############################
with st.expander(":robot_face: Simulating 1000 season endings... :robot_face:", expanded=False):
    st.subheader("1000 Simulated Season Ends")
    st.write("*1000 simulated season endings are generated by modeling the outcome of all remaining fixtures, over and over again. This approach incorporates both individual player histories and league-wide baselines, applying a form of regression to the mean that tempers early-season extremes. Instead of locking a player into perpetually achieving their rare peaks or  lows, each player's score distribution is relevant to themn, but nudged towards the broader league average, reflecting a belief that extraordinary performances may normalize over time.*")
    st.write("")
    st.write("*A truncated normal distribution, slightly extended beyond historical bounds, ensures that while improbable scores remain rare, they are not impossible, maintaining a realistic spread of outcomes. Each player's mean and standard deviation blend their own data with the league's, so that a handful of lucky or unlucky weeks do not disproportionately dictate future potential. By repeatedly (1000 ties) drawing from these refined distributions, the simulations produce a probability landscape for final standings that recognizes both individual variance and the shared statistical tendencies of the entire cohort. In essence, this methodology provides a more balanced and subtle perspective, where even if a manager leads early, there's meaningful probability that others may close the gap, and if a player has been unlucky, there's a tangible chance of improvement. As a result, the model yields - in theory - a broad tapestry of plausible season conclusions. This shows us the all those possible outcomes, and how likely they actually are across 1000 seasons.*")
